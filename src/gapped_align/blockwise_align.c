...
static BOOL
calc_block_size(const idx block_size, idx qleft, idx tleft, idx* qsize, idx* tsize)
{
	const idx max_block_size = 1.5 * block_size;
	BOOL last_block = 0;
	   ...
	   int right_extend)
{
	...
	*dist = 0;
	while (1) {
		idx qblk, tblk;
		BOOL is_done = calc_block_size(block_size, qleft, tleft, &qblk, &tblk);
		if (qblk == 0 || tblk == 0) break;
		extract_subsequence(reads, qid, qdir, qstart, qblk, right_extend, query);
		extract_subsequence(reference, tid, tdir, tstart, tblk, right_extend, target);
