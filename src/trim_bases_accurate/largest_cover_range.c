...
#include "../consensus/consensus_one_read.h"

#include <assert.h>

static const int kMaxM4PerRead = 300;
...
lcr_worker(void* arg)
{
	...
	while ((m4v = get_next_range(data, &nm4))) {
		if (data->cns_reads) {
			if (nm4 > kMaxM4PerRead) {
				nm4 = kMaxM4PerRead;
			} else {
				ks_introsort_M4Record_IdentGT(nm4, m4v);
			}
		} else {
			truncate_m4_list(m4v, &nm4, kMaxM4PerRead, data->min_ident_perc);
			ks_introsort_M4Record_IdentGT(nm4, m4v);
		}
		...
		const int num_threads)
{
	...
	if (reads == NULL) cns_reads = cns_reads_new_m4(kv_data(m4v), kv_size(m4v), kMaxM4PerRead, min_ident_perc, pac_reads_dir);
