...
static const int kMaxEnd = 20;

int lcr_is_complete(LargestCoverRange* lcr)
{
  int r = lcr_is_valid(*lcr) && (lcr->left <= kMaxEnd) && (lcr->size - lcr->right <= kMaxEnd);
  return r;
}
...
void load_lcrs(const char* path, vec_lcr* lcrv)
{
  ...
  int read_id;
  kv_clear(*lcrv);
  while (kgetline(&line, fgets, in) != EOF) {
	  kputc('\0', &line);
	  SAFE_SCANF(sscanf, kstr_str(line), 4, "%d%d%d%d", &read_id, &lcr.left, &lcr.right, &lcr.size);
	  kv_push(LargestCoverRange, *lcrv, lcr);
	  kstr_clear(line);
  }
  FCLOSE(in);
  free_kstring(line);
...
lcr_worker(void* arg)
{
	...
	LargestCoverRange* v_lcr = data->lcrv;
	LargestCoverRange lcr;
	
	while ((m4v = get_next_range(data, &nm4))) {
		truncate_m4_list(m4v, &nm4, data->min_ident_perc);
		if (is_complete_read(m4v, nm4, data->min_ident_perc, &left, &right)) goto set_clipped_range;
		if (is_chimeric_read(m4v, nm4, data->min_ident_perc, &left, &right)) goto set_clipped_range;
